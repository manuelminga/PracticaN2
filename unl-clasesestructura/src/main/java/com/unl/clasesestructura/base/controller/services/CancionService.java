...
public class CancionService {
     ...
     * @throws ListEmptyException Si no hay canciones registradas.
     */
    public List<HashMap> listCancion() throws ListEmptyException {
        List<HashMap> lista = new ArrayList<>();

        if (!dc.listAll().isEmpty()) {
            Cancion[] arreglo = dc.listAll().toArray();

            // Por cada canción, agrega datos extendidos (género, álbum)
            for (int i = 0; i < arreglo.length; i++) {
                HashMap<String, String> aux = new HashMap<>();
                aux.put("id", arreglo[i].getId().toString());
